...
Measured on the same 16-core WSL2 machine with a real TCP client that verifies every reply; persistence disabled. Rudis runs the optimized build (`lto = "thin"`, `codegen-units = 1`, `panic = "abort"`, mimalloc, `target-cpu=native`) — see `[profile.release]` in `Cargo.toml` and `.cargo/config.toml`. Results in millions of ops/s:
...
† Garnet figures are medians across runs — GC pauses swing results by up to 7×, so the 3.4M write outliers above are the low end; Rudis reproduces within ~3%. Valkey runs at its optimum: enabling `io-threads` made it *slower* on this machine. `valkey-benchmark` is not used because its single-threaded client caps at ~9M req/s and underreports Rudis.

Rudis scales with connections (2.5M → 21M+); Valkey's single-threaded loop tops out at 2–3M.
